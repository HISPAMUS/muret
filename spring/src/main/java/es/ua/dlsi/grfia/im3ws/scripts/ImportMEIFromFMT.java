...
public class ImportMEIFromFMT implements CommandLineRunner {
    ...
    private static final String DOCUMENT = "C5";
    ...
    public void run(String... args) throws Exception {
        ...
        if (args.length == 2) {
            ...
            throw new Exception("Use: <MEI folders path> <collection name>");
        }
        ...
        // first check all data are correct
        for (File file: meiFiles) {
            ...
            if (staves.size() != pageSystemBeginningsMap.size()) {
                throw new Exception("MuRET has " + staves.size() + "staves  and MEI file has " + pageSystemBeginnings.getSystemBeginnings());
            }
    ...
    private void importStaffContent(int regionNumber, Region region, Sequence<AgnosticToken> agnosticTokenSequence, SemanticEncoding semanticEncoding) throws Exception {
        ...
        int symbolWidth = region.getBoundingBox().getWidth() / agnosticTokenSequence.size();
        int currentApproxX = 0;
