 ...
 */
public class DocumentScoreSong {
    ScoreSong scoreSong;
     ...
     * key = part name
     */
    LinkedHashMap<String, DocumentScoreSongPart> parts;

    public DocumentScoreSong(File file, NotationType notationType) throws IM3WSException {
    ...
    public DocumentScoreSong(ScoreSong scoreSong) throws IM3WSException {
        this.scoreSong = scoreSong;
        if (scoreSong.getFacsimile() == null) {
            throw new IM3WSException("The document score song needs a score song with facsimile");
        }
        readParts();
    }
    ...
    private void readParts() throws IM3WSException {
        ...
        for (ScorePart scorePart: scoreSong.getParts()) {
            ...
            parts.put(scorePart.getName(), new DocumentScoreSongPart(scorePart));
        }
    }


    public DocumentScoreSongPart getScorePart(String partName) {
        return this.parts.get(partName);
    }

    public DocumentScoreSongPart addPart(String partName) throws IM3WSException {
        if (this.parts == null) {
            this.parts = new LinkedHashMap<>();
        }
        ...
        DocumentScoreSongPart documentScoreSongPart = new DocumentScoreSongPart(scorePart);
        this.parts.put(partName, documentScoreSongPart);
        this.save();
        return documentScoreSongPart;
    }

    public Collection<DocumentScoreSongPart> getScoreParts() {
        if (this.parts == null) {
            return null;
        } else {
