...
import {LineComponent} from '../../../drawing/components/svgcanvas/components/line/line.component';
import {Rectangle} from '../../../drawing/components/svgcanvas/model/shape';
import {RegionType} from '../../../model/region-type';
import {RectangleComponent} from '../../../drawing/components/svgcanvas/components/rectangle/rectangle.component';
import {RegionTypesCrudService} from "../../../services/crud/region-types-crud.service";
import {ImageCrudService} from "../../../services/crud/image-crud.service";
import {DocumentAnalysisService} from "../../../services/businesslogic/document-analysis.service";
...
export class DocumentAnalysisViewComponent implements OnInit, AfterViewInit {
  ...
  constructor(
    private imageService: ImageCrudService, //TODO Quitar
    private regionTypesCrudService: RegionTypesCrudService, //TODO Quitar
    ...
    if (!sessionDataService.regionTypes) {
      this.regionTypesCrudService.getRegionTypes().subscribe(value => {
  ...
  private splitPage(imageX: number) {
    ...
    try {
      this.documentAnalysisService.splitPage$(this.image.id, imageX).subscribe(next => {
  ...
  private splitRegion(imageX: number, imageY: number) {
    ...
    this.canvasCursor = 'wait';
    try {
      this.documentAnalysisService.splitRegion$(this.image.id, imageX, imageY).subscribe(next => {
        this.image.pages = next;
        this.drawImagePages();
        this.canvasCursor = prevCursor;
        this.clearInteractiveLines();
  ...
  public clearDocumentAnalysis() {
    this.documentAnalysisService.clearDocumentAnalysis$(this.image.id).subscribe(next => {
      this.image.pages = next;
      this.drawImagePages();
      this.clearInteractiveLines();
    });
  ...
  onShapeChanged($event: ShapeComponent) {
    ...
    if ($event.shape instanceof Rectangle) {
      ...
      if ($event.modelObjectType === 'Region') {
        this.documentAnalysisService.updateRegionBoundingBox$($event.modelObjectID,
        ...
        this.documentAnalysisService.updatePageBoundingBox$($event.modelObjectID,
          $event.shape.originX / this.scale, $event.shape.originY / this.scale,
          ($event.shape.originX + $event.shape.width)  / this.scale,
          ($event.shape.originX + $event.shape.height) / this.scale);
      }
  ...
  changeRegionType(regionType: RegionType) {
    if (this.selectedRegion != null) {
      ...
      if (region) {
        this.documentAnalysisService.updateRegionType$(region.id, regionType).subscribe(next => {
